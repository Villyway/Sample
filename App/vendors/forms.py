...
class VendorForm(forms.Form):
    ...
    code = forms.CharField(required=True, label="Code/Sku", widget=forms.TextInput(
        attrs={"class": "form-control"}))
    name = forms.CharField(required=False, label="Name", widget=forms.TextInput(
        attrs={"class": "form-control"}))
    mobile = forms.CharField(
        required=False, max_length=10, label="Mobile", widget=forms.NumberInput(attrs={"class": "form-control", "type": "tel"}), validators=[mobile_regex])
    email = forms.EmailField(label='Email', widget=forms.EmailInput(
        attrs={'class': 'form-control'}), required=False, validators=[validate_email])
    gst_no = forms.CharField(required=False, label="Name", widget=forms.TextInput(
        attrs={"class": "form-control"}))
